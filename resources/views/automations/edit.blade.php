...
<form action="{{ isset($automation) ? route('automations.update', $automation) : route('automations.store') }}" method="POST" class="max-w-lg space-y-4">
    ...
    <div>
        ...
        <input type="text" name="trigger_event" value="{{ old('trigger_event', $automation->trigger_event ?? '') }}" required
            placeholder="e.g. lead_created, task_due" class="w-full px-3 py-2 border rounded" />
    </div>
